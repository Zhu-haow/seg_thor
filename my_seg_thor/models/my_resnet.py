...
class PSPDec(nn.Module):
    ...
    def __init__(self, nIn, nOut, downSize):
        ...
        # self.up = nn.Upsample(size=inp_size[2], mode='bilinear')

    def forward(self, x):
        assert x.dim() == 4
        inp_size = x.size()
        # out_dim1, out_dim2 = int(inp_size[2] * self.scale), int(inp_size[3] * self.scale)
        out_dim1, out_dim2 = int(self.scale), int(self.scale)

        x_down = F.adaptive_avg_pool2d(x, output_size=(out_dim1, out_dim1))
        # return F.upsample(self.features(x_down), size=(64, 64), mode='bilinear')
...
class ResNetC1(nn.Module):
    ...
    def __init__(self):
        ...
        self.p01 = PSPDec(16 + classes, classes, 160)
        self.p02 = PSPDec(16 + classes, classes, 128)
        self.p03 = PSPDec(16 + classes, classes, 96)
        self.p04 = PSPDec(16 + classes, classes, 72)
        ...
        self.p10 = PSPDec(classes + 256, 64, 80)
        self.p20 = PSPDec(classes + 256, 64, 64)
        self.p30 = PSPDec(classes + 256, 64, 48)
        self.p40 = PSPDec(classes + 256, 64, 36)

        self.class_1 = nn.Sequential(
            nn.Conv2d(5 + 256 + 64 * 4, classes, 3, padding=1, bias=False),
            nn.BatchNorm2d(classes, momentum=0.95, eps=1e-3),
            nn.ReLU(True),
        ...
        self.class_conv = nn.Sequential(
            ...
            nn.Conv2d(classes, dia_class, kernel_size=1, stride=1),
    ...
    def forward(self, input1):
        ...
        output3 = self.upsample_3(output3)      #shape[1] = 5
        ...
        seg_out = self.upsample_1(output0_hook)
        class_out = self.class_conv(output0_hook)

        return seg_out, class_out.view(class_out.size(0), class_out.size(1))