...
parser.add_argument(
    ...
    type=str,
    metavar='PATH',
    help='path to load pretrained checkpoint (default: none)')
parser.add_argument(
    '--weight-decay',
    '--wd',
    default=0.00001,
...
def main(args):
    ...
    tr.CenterCrop(384),
    tr.RandomZoom((384, 384)),
    tr.RandomRotate(30),
    tr.RandomHorizontalFlip(),
    tr.Normalize(mean=(0.12, 0.12, 0.12), std=(0.018, 0.018, 0.018)),
    tr.ToTensor2(args.n_class)])

    train_dataset = THOR_Data(transform=composed_transforms_tr, path=data_path, file_list=train_files)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        eval_dice, eval_precision = evaluation(args, net, loss, epoch, save_dir, test_files, adaptive_thresholds)
...
def train(data_loader, net, loss, epoch, optimizer, get_lr, save_dir):
    ...
    for i, sample in enumerate(data_loader):
        ...
        total_train_s_loss.append(s_loss.item())
    ...
    logging.info(
        'Epoch[%d], Batch [%d], total train loss is %.6f, s_loss is %.6f, using %.1f s!' %
        (epoch, i, np.mean(total_train_loss),np.mean(total_train_s_loss), time.time() - start_time))
...
def evaluation(args, net, loss, epoch, save_dir, test_files, saved_thresholds):
    ...
    logging.info(
        'Epoch[%d], [total eval loss=%.6f],[ eval s_loss=%.6f], mean_dice=%.4f '
        % (epoch, np.mean(eval_loss),np.mean(eval_s_loss), np.mean(dices)))
    logging.info(
        '***************************************************************************\n'
    )
